...
if [ "$prev" = "-m" ]; then
   ...
   COMPREPLY=( $( compgen -W 'absmean change count iqr max mean meanabs median min quantile range std sum variance' -- $cur ) )
elif [ "$prev" = "-type" ]; then
