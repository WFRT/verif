...
if [ "$cur" = "" ] || [[ "$cur" =~ -* ]]; then
   COMPREPLY=( $( compgen -f -W ' -m --config --list-times --list-locations --list-quantiles --list-thresholds --version -d -elevrange -l -lx -latrange -lonrange -o -r -q -t -x -acc -agg -b -c -C -fcst -hist -obs -sort -aspect -bottom -clim -cmap -dpi -f -fs -labfs -lc -left -leg -legfs -legloc -ls -lw -maptype -ms -nogrid -nomargin -right -simple -sp -tickfs -title -titlefs -top -type -xlabel -xlim -xlog -xrot -xticks -xticklabels -ylabel -ylim -ylog -yrot -yticks -yticklabels ' -- $cur ) )
fi
if [ "$prev" = "-m" ]; then
   COMPREPLY=( $( compgen -W ' a  against  alphaindex  autocorr  b  baserate  bias  biasfreq  bs  bsrel  bsres  bss  bsunc  c  change  cmae  cond  corr  d  derror  diff  discrimination  dmb  droc  droc0  drocnorm  dscore  economicvalue  edi  eds  ef  error  ets  fa  far  fcst  fcstrate  freq  fss  hit  hss  ign0  igncontrib  impact  invreliability  kendallcorr  kss  leps  lor  mae  marginal  marginalratio  mbias  meteo  miss  n  nsec  obs  obsfcst  or  pc  performance  pit  pithist  pithistdev  pithistshape  pithistslope  qq  quantilescore  rankcorr  reliability  rmse  rmsf  roc  scatter  sedi  seds  spherical  spreadskill  stderror  taylor  threat  timeseries  within  yulesq  ' -- $cur ) )
elif [ "$prev" = "-cmap" ]; then
