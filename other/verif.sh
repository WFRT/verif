...
if [ "$prev" = "-m" ]; then
   COMPREPLY=( $( compgen -W 'a adjustedensemblemeanrmse against alphaindex autocorr autocov b baserate bias biasfreq bs bsdecomp bsrel bsres bss bssrel bssres bsunc c change cmae cond corr d derror diff discrimination dmb droc droc0 dscore economicvalue edi eds ef ensemblesamplevariance error ets fa far fcrps fcst fcstrate fcststddev freq fss hit hss ign0 igncontrib invreliability kendallcorr kge kss leps lor mae marginal marginalratio mbias meteo miss murphy n nnsec nsec obs obsfcst obsstddev or pc performance pit pithist pithistdev pithistshape pithistslope qq quantile quantilecoverage quantilescore rankcorr rankhist ratio reliability rmse rmsf roc scatter sedi seds spherical spread spreadandskill spreadskill spreadskillratio stderror taylor threat threshold timeseries within yulesq ' -- $cur ) )
elif [ "$prev" = "-cmap" ]; then
