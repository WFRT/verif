...
def show_description(data=None):
   ...
   s += format_argument("-x dim", "Plot this dimension on the x-axis: date, leadtime, year, month, week, day, location, elev, lat, lon, threshold, leadtimeday, or no. Not supported by all metrics. If not specified, then a default is used based on the metric. 'location' refers to the location id. 'leadtimeday' aggregates leadtimes into whole forecast days. 'no' collapses all dimensions and computes one value.") + "\n"
